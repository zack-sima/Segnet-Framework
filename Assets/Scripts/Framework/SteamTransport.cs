...
public class SteamTransport : MonoBehaviour, ITransport {
    ...
    // We support multiple connections even if you only use one initially
    ...
    public int MaxPacketSize { get; private set; } = 1200; // safe MTU-ish
    ...
    private void ReceiveAllMessages() {
        ...
        // For now just iterate all active connections
            ...
            while (true) {
                ...
                for (int i = 0; i < msgCount; i++) {
                    ...
                    if (msg.m_cbSize > 0 && msg.m_pData != IntPtr.Zero) {
                        ...
                        // Steam doesn't expose channel type on receive directly; you can
                        // track per-message if you want. For now we treat as Unreliable.
